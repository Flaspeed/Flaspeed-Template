...
if(!isMobileTooltip){
    (function() {
        ...
        Tooltip.prototype.attachEvents = function(targetEl, tooltipEl, backdropEl) {
            ...
            const showTooltip = () => {
                // تحديث الموضع قبل إظهار التلميح في كل مرة
            ...
            // استمع إلى تحريك المؤشر فوق العنصر
            ...
            // استمع إلى تحرك المؤشر أثناء الوقوف على العنصر لتحديث الموضع
            targetEl.addEventListener('pointermove', () => {
                if (isVisible) {
                    // تحديث الموضع فوراً عند تحرك المؤشر إذا كان التلميح مرئياً
                    const position = this.getPosition(targetEl);
                    const { left, top, translateX, translateY } = adjustPosition(targetEl, tooltipEl, backdropEl, position);
            ...
            // استمع إلى حدث تغيير حجم النافذة لتحديث موضع التلميح إذا كان مرئياً
