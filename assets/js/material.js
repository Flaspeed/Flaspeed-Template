...
function initDropdown(elements, options = {}) {
  ...
  for (let i = 0; i < elements.length; i++) {
    ...
    function placeDropdown(eventType) {
      ...
      activates.style.top = (origin.offsetTop + verticalOffset + scrollYOffset + 5) + 'px';
    ...
    if (activates) {
      // إضافة مستمع أحداث للروابط والأزرار التي لا تحتوي على الكلاس sp-btn
      activates.addEventListener('click', function(e) {
        // تحقق ما إذا كان العنصر المنقور عليه هو رابط <a> أو زر <button> وليس له الكلاس sp-btn
        const clickedElement = e.target.closest('a, button');
        if (clickedElement && !clickedElement.classList.contains('sp-btn')) {
          // إغلاق القائمة فور النقر على العنصر
          hideDropdown();
        }
      });

      // نحتفظ بالسلوك السابق للأزرار التي تحمل الكلاس sp-btn
      const buttons = activates.querySelectorAll('.sp-btn');
