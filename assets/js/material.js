...
if(!isMobileTooltip){
    (function() {
        ...
        Tooltip.prototype.init = function(element) {
            ...
            tooltipEl.style.zIndex = '9999'; // High z-index to stay on top
        ...
        Tooltip.prototype.updatePosition = function(targetEl, tooltipEl, backdropEl) {
        ...
        Tooltip.prototype.attachEvents = function(targetEl, tooltipEl, backdropEl) {
            ...
            // Store tooltip data for resize handling
            ...
            const showTooltip = () => {
                // Make sure elements are visible before measuring
                tooltipEl.style.visibility = 'visible';
                backdropEl.style.visibility = 'visible';
                tooltipEl.style.opacity = '0'; // Hide initially for measurement
                
                // Calculate position immediately before showing
                const { translateX, translateY } = this.updatePosition(targetEl, tooltipEl, backdropEl);
                ...
                isVisible = true;
                this.activeTooltips.get(tooltipEl.id).isVisible = true;
            };
            ...
            const hideTooltip = () => {
                ...
                setTimeout(() => {
                    if (!isVisible) {
                        ...
                        this.activeTooltips.get(tooltipEl.id).isVisible = false;
                    }
                    isVisible = false;
                }, 225);
            };
            ...
            const handleResize = () => {
                ...
                resizeTimeout = setTimeout(() => {
                    if (isVisible) {
                        this.updatePosition(targetEl, tooltipEl, backdropEl);
                    }
                }, 100);
            };
            ...
            // For touch interactions - also handle touch start
            targetEl.addEventListener('touchstart', (e) => {
                // Don't conflict with click events
                clearTimeout(hoverTimeout);
                hoverTimeout = setTimeout(() => {
                    showTooltip();
                }, this.getDelay(targetEl));
        ...
        document.addEventListener('visibilitychange', () => {
            if (document.visibilityState === 'visible') {
                // Update positions of all visible tooltips when page becomes visible again
                tooltipInstance.activeTooltips.forEach((tooltipData, id) => {
                    if (tooltipData.isVisible) {
                        tooltipInstance.updatePosition(
                            tooltipData.targetEl, 
                            tooltipData.tooltipEl, 
                            tooltipData.backdropEl
                        );
                    }
                });
            }
        ...
        // Handler for orientation change (mobile)
        window.addEventListener('orientationchange', () => {
            // Hide any visible tooltips during orientation change
            tooltipInstance.activeTooltips.forEach((tooltipData, id) => {
                if (tooltipData.isVisible) {
                    tooltipData.tooltipEl.style.visibility = 'hidden';
                    tooltipData.backdropEl.style.visibility = 'hidden';
                    
                    // After orientation change completes, update and show again
                    setTimeout(() => {
                        if (tooltipData.isVisible) {
                            tooltipInstance.updatePosition(
                                tooltipData.targetEl, 
                            ...
                            tooltipData.tooltipEl.style.visibility = 'visible';
                            tooltipData.backdropEl.style.visibility = 'visible';
                        }
                    }, 300);
                }
            });
        ...
        window.VanillaTooltip = function(selector, options) {
            if (options === 'remove') {
                ...
                if (tooltipId) {
                    ...
                    if (tooltipEl) {
                        tooltipEl.remove();
                        tooltipInstance.activeTooltips.delete(tooltipId);
                    }
            ...
            return tooltipInstance.init(selector);
        };
    })();
};
/*DropMenu*/
