...
if(!isMobileTooltip){
    (function() {
        ...
        // نستخدم position: fixed لذا نأخذ الإحداثيات من getBoundingClientRect مباشرة
        ...
        // Boundary checks
        if (left < 4) left = 4;
        if (left + tooltipRect.width > windowWidth - 4) {
        left = windowWidth - tooltipRect.width - 4;
        }
        if (top < 4) top = 4;
        if (top + tooltipRect.height > windowHeight - 4) {
        ...
        // إزالة أي tooltip قديم
        const oldId = element.getAttribute('data-tooltip-id');
        if (oldId) {
        ...
        if (this.isHtml(element)) contentEl.innerHTML = text;
        ...
        targetEl, tooltipEl, backdropEl, pos
        );
        ...
        // حساب مقاييس جديدة بعد تغيير النص/مواقع لـ backdrop
        const tooltipW = tooltipEl.offsetWidth;
        const tooltipH = tooltipEl.offsetHeight;
        const backdropW = backdropEl.offsetWidth;
        const backdropH = backdropEl.offsetHeight;
        const scale = Math.max(
        (Math.SQRT2 * tooltipW) / backdropW,
        (Math.SQRT2 * tooltipH) / backdropH
        );
        ...
        hoverTimeout = setTimeout(show, this.getDelay(targetEl));
        });
        ...
        const t = new Tooltip(options);
        return t.init(selector), selector;
        };
        })();
