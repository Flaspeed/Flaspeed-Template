...
(function() {
    ...
    function Tooltip(options) {
        this.defaultOptions = {
            ...
            position: 'bottom',
            html: false,
            trackPosition: true  // New option to track position
        };
        this.options = Object.assign({}, this.defaultOptions, options);
        this.activeTooltips = {};  // Track active tooltips
    }
    ...
    Tooltip.prototype.trackPosition = function(element) {
        return element.getAttribute('data-track-position') !== 'false' && this.options.trackPosition;
    };
    ...
    Tooltip.prototype.attachEvents = function(targetEl, tooltipEl, backdropEl) {
        ...
        const showTooltip = () => {
            ...
            if (this.trackPosition(targetEl)) {
                positionInterval = setInterval(updatePosition, 100);
                
                // Store reference to interval in active tooltips
                this.activeTooltips[tooltipEl.id] = {
                    interval: positionInterval,
                    element: targetEl
                };
                
                // Also track on scroll and resize
        ...
        const hideTooltip = () => {
            ...
            if (positionInterval) {
                ...
                delete this.activeTooltips[tooltipEl.id];
    ...
    window.VanillaTooltip = function(selector, options) {
        if (options === 'remove') {
            ...
            if (tooltipId) {
                ...
                if (tooltipInstance.activeTooltips[tooltipId]) {
                    clearInterval(tooltipInstance.activeTooltips[tooltipId].interval);
                    delete tooltipInstance.activeTooltips[tooltipId];
                }
            }
        ...
        // If options is provided, create a new instance with those options
        const tooltip = options ? new Tooltip(options) : tooltipInstance;
